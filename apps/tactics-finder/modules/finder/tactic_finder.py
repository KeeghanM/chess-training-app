...
class TacticFinder:
    ...
    def get_evaluations_from_best_moves(self, best_moves: Optional[list[dict]] = None) -> list[Evaluation]:
    ...
    def is_position_hard(self, best_moves: Optional[list[dict]] = None) -> bool:
        if best_moves is None:
            best_moves = self.stockfish.get_top_moves(self.stockfish_top_moves)
        evaluations: list[Evaluation] = self.get_evaluations_from_best_moves(best_moves)
        if len(best_moves) <= 1:
            return False
        elif len(best_moves) >= 2:
            first_evaluation: Evaluation = evaluations[0] if self.white else -evaluations[0]
            second_evaluation: Evaluation = evaluations[1] if self.white else -evaluations[1]
            if first_evaluation.mate and second_evaluation.mate:
                return first_evaluation.value > 0 > second_evaluation.value or (
                    first_evaluation.value > 0
                    and second_evaluation.value > 0
                    and abs(first_evaluation.value - second_evaluation.value) > 5
                )
            elif first_evaluation.mate and not second_evaluation.mate:
                return first_evaluation.value > 0
            elif not first_evaluation.mate and second_evaluation.mate:
                return first_evaluation.value >= 0.0
            else:
                return first_evaluation.value > 0 > second_evaluation.value
        else:
            raise ValueError("unexpected number of moves")

    def is_only_one_good_move(self, best_moves: Optional[list[dict]] = None) -> bool:
        if best_moves is None:
            best_moves = self.stockfish.get_top_moves(self.stockfish_top_moves)
        evaluations: list[Evaluation] = self.get_evaluations_from_best_moves(best_moves)
        if len(best_moves) == 0:
            ...
            evaluation: Evaluation = evaluations[0] if self.white else -evaluations[0]
            return evaluation.value > 0
        elif len(best_moves) >= 2:
            first_evaluation: Evaluation = evaluations[0] if self.white else -evaluations[0]
            second_evaluation: Evaluation = evaluations[1] if self.white else -evaluations[1]
            if first_evaluation.mate and second_evaluation.mate:
                return first_evaluation.value > 0 and first_evaluation > second_evaluation
            elif first_evaluation.mate and not second_evaluation.mate:
                return first_evaluation.value > 0
            elif not first_evaluation.mate and second_evaluation.mate:
                return first_evaluation.value >= 0.0
            elif not first_evaluation.mate and not second_evaluation.mate:
                return (
                    first_evaluation.value - second_evaluation.value > self.pawn_threshold
                    and 0 <= first_evaluation.value <= self.pawn_limit
                )
        else:
            raise ValueError("unexpected number of moves")
        return False

    def get_good_enough_moves(self, best_moves: Optional[list[dict]] = None) -> list[str]:
        if best_moves is None:
            best_moves = self.stockfish.get_top_moves(self.stockfish_top_moves)
        if best_moves is None:
            return []
        if len(best_moves) == 0:
            ...
            best_evaluation: Evaluation = next(iter(evaluations.values()))
    ...
    def find(
        ...
        fen: str = self.stockfish.get_fen_position()
        self.visited_fens.add(fen)
        if fen in self.fens:
            raise PositionOccurred("position already occurred")

        best_moves: list[dict] = self.stockfish.get_top_moves(self.stockfish_top_moves)
        material_balance: int = self.get_relative_material_balance(fen)
        color: bool = self.white ^ defender
        forced: bool = len(best_moves) == 1 and self.stockfish_top_moves > 1
        hard: bool = defender or self.is_position_hard(best_moves)
        ...
        if move is None:
            position: Position = self.starting_position
            position.forced = forced
            position.hard = hard
        else:
            if evaluation is None:
                # This should not happen if there are best_moves
        ...
        node: Node = Node(position, parent=parent)
        board: chess.Board = self.get_board_from_history(node)
        outcome: Outcome = self.get_outcome(board, evaluation, material_balance)
        ...
        if outcome.type == "draw":
            ...
            if defender:
                good_enough_responses: list[str] = self.get_good_enough_moves(best_moves)
                new_fen: str = self.stockfish.get_fen_position()
                ...
                if self.is_only_one_good_move(best_moves):
                    best_move: str = best_moves[0]["Move"]
    ...
    def get_outcome(self, board: chess.Board, evaluation: Optional[Evaluation], material_balance: int) -> Outcome:
    ...
    def get_variations(self, headers: Optional[Headers] = None) -> Tuple[Optional[Variations], Optional[Tactic]]:
        root: Optional[Node] = self.create_tree()
        if root and root.children:
            variations: Variations = Variations(root, headers=headers)
            tactic: Optional[Tactic] = variations.get_tactic()
